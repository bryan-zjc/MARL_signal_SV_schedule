...
from src.road_env.env_network_PL import Intersec_Env
        ...
        while not done:
            ...
            if num_act_agent > 0:
                for tls_id in act_agent_ids:
                    ...
                    if agent_select == 'DQN':
                        action[tls_id] = agent.get_action_online(states[tls_id], eps_list[tls_id])
                ...
                for tls_id in act_agent_ids:
                    if len(traj_last[tls_id]) != 0:
                        ...
                        if buffer._size > sample_size:
                            ...
                            # Train the agent
                            if agent_select == 'DQN':
                                loss, cql_loss, bellman_error = agent.learn(buffer.sample(sample_size))
                            elif agent_select == 'SAC':
...
if __name__ == "__main__":
    ...
    if mode_online_train:
        ...
        for i in inters_list:
            if agent_select == 'DQN':
                agent_list[str(i)]=DQNAgent(state_size=state_size, action_size=action_size, hidden_size=256, device=device, agent_select=agent_select)
            elif agent_select == 'SAC':
