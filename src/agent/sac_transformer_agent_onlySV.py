# -*- coding: utf-8 -*-
"""
Created on Sat Feb 15 09:38:19 2025
...
from src.network.networks_sac_transformer_onlySV import Critic, Actor
import numpy as np
import math
import copy
import random
...
class SACTFAgent(nn.Module):
    ...
    def __init__(self,
        ...
        # Actor Network 
        self.actor_local = Actor(RV_state_size, seq_len, action_size, hidden_size).to(device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=self.lr)     
        
        # Critic Network (w/ Target Network)
        self.critic1 = Critic(RV_state_size, seq_len, action_size, hidden_size, 2).to(device)
        self.critic2 = Critic(RV_state_size, seq_len, action_size, hidden_size, 1).to(device)
        
        assert self.critic1.parameters() != self.critic2.parameters()
        
        self.critic1_target = Critic(RV_state_size, seq_len, action_size, hidden_size).to(device)
        self.critic1_target.load_state_dict(self.critic1.state_dict())

        self.critic2_target = Critic(RV_state_size, seq_len, action_size, hidden_size).to(device)
    ...
    def get_action_online(self, RV_state):
        """Returns actions for given state as per current policy."""
        # state = torch.from_numpy(state).float().to(self.device)
        RV_state = torch.from_numpy(np.array(RV_state)).float().to(self.device)
        with torch.no_grad():
            action = self.actor_local.get_det_action(RV_state)
        return int(action)

    def calc_policy_loss(self, RV_states, alpha):
        _, action_probs, log_pis = self.actor_local.evaluate(RV_states)

        q1 = self.critic1(RV_states)   
        q2 = self.critic2(RV_states)
    ...
    def learn(self, experiences):
        ...
        """
        RV_states, actions, rewards, next_RV_states, dones = experiences

        # ---------------------------- update actor ---------------------------- #
        current_alpha = copy.deepcopy(self.alpha)
        actor_loss, log_pis = self.calc_policy_loss(RV_states, current_alpha)
        ...
        with torch.no_grad():
            _, action_probs, log_pis = self.actor_local.evaluate(next_RV_states)
            Q_target1_next = self.critic1_target(next_RV_states)
            Q_target2_next = self.critic2_target(next_RV_states)
