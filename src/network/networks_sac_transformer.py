...
class Actor(nn.Module):
    def __init__(self, RV_state_size, PL_state_size, seq_len, action_size, hidden_size):
        ...
        # Policy network
        state_size = seq_len+PL_state_size
        self.fc_policy = nn.Linear(state_size, action_size)
        self.softmax = nn.Softmax(dim=-1)
        self.reset_parameters()
        self.seq_len = seq_len

    def forward(self, RV_state, PL_state):
        ...
        if features.dim() == 2:
            ...
            features = features.mean(dim=-1)
        new_state = torch.cat((features, PL_state), dim=-1)
    ...
    def evaluate(self, RV_state, PL_state, epsilon=1e-6):
        action_probs = self.forward(RV_state, PL_state)
    ...
    def get_action(self, RV_state, PL_state):
        """
    ...
    def get_det_action(self, RV_state, PL_state,):
        action_probs = self.forward(RV_state, PL_state)
        dist = Categorical(action_probs)
        action = dist.sample().to(RV_state.device)
...
class Critic(nn.Module):
    def __init__(self, RV_state_size, PL_state_size, seq_len, action_size, hidden_size, seed=1):
