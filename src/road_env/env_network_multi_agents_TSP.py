...
class Intersec_Env:
    ...
    def reset(self,RV_schedule, save_path, episode):
        ...
        self.get_lane_3RVs(RV_schedule)
    ...
    def get_RV_state(self,tls_id,RV_schedule):
        ...
        for lane in self.net_info[tls_id]['lanes_in_no_right']:
            # do not consider the right turnning movement
            RV_list = self.lane_RV_top3[lane]
            for i in range(3):
                try:
                    ...
                    RV_state.append(0)
                    RV_state.append(300) # far away from the stop line
        # RV state dim = 8*3 + 8*3
        return RV_state
    ...
    def _collect_RV_reward(self, tls_id):
        ...
        for i in self.inter_RV_last[tls_id]:
            if i in self.schedule_delay.keys():
                RV_r += (abs(self.schedule_delay_last[i])-abs(self.schedule_delay[i]))/self.RV_remain_inters_last[i]
    ...
    def get_lane_3RVs(self,RV_schedule):
        ...
        self.RV_eta = {} # eta可以考虑用神经网络拟合，先简单写一个估计方法
        self.schedule_delay = {}
        ...
        self.lane_RV_top3 = copy.deepcopy(self.lane_RV)
        lanes = list(self.lane_RV.keys())
        for l in lanes:
            if len(self.lane_RV[l]) > 3:
                delay_distance_list = []
                ...
                top_three = delay_distance_sort[:3]
                top_three_indices = [x[0] for x in top_three]
                
                self.lane_RV_top3[l] = []
                for x in top_three_indices:
                    self.lane_RV_top3[l].append(self.lane_RV[l][x])
